...
export default function EntryModal({ al, plots, breeds, photoBase64, onSaved, onClose, toast }) {
  ...
  // Generate the DO number once on open.
  useEffect(() => {
    generateDONumber()
      .then(setDoNumber)
  ...
  // Run the AI scan when a photo is supplied.
  useEffect(() => {
  ...
  function addRow() {
    setRows((rs) => (rs.length >= 5 ? (toast('Maximum 5 item rows'), rs) : [...rs, emptyRow()]));
  }
  ...
  async function save() {
    ...
    if (!items.length) return alert('Please add at least one item row.');
    if (totalQty <= 0) return alert('Total Qty must be greater than 0.');
    if (totalQty > balance) return alert(`Total Qty (${totalQty}) exceeds remaining balance (${balance}).`);
    if (!sigRef.current?.hasSig()) return alert('Please ask the customer to sign before saving.');
        ...
        <div className="p-6 rounded-t-[24px] flex justify-between items-start" style={{ background: 'linear-gradient(135deg,#0f172a,#1e293b)' }}>
          <div>
            <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">
              {photoBase64 ? '📷 AI-Powered Scan' : '✏️ Manual Entry'}
            </div>
            ...
            <>
              ...
              <div>
                <div className="flex justify-between items-center mb-2">
                  ...
                  {rows.length < 5 && (
                    <button onClick={addRow} className="text-[9px] font-black text-blue-700 bg-blue-50 hover:bg-blue-100 border border-blue-200 px-3 py-1.5 rounded-lg cursor-pointer uppercase tracking-widest">+ Add Row</button>
                  )}
                </div>
                    ...
                    <tbody>
                      {rows.map((r, i) => (
                        <tr key={r.key}>
                          ...
                          <td className="p-1.5">
                            <input list="nursery-list" value={r.nursery} onChange={(e) => updateRow(r.key, 'nursery', e.target.value)} placeholder="Nursery / Farm" className="search-input text-xs w-full" style={{ padding: '7px 10px' }} />
                            {r.ai && <span className="text-[9px] text-emerald-600 font-black">✨AI</span>}
                          </td>
                          <td className="p-1.5">
                            <input list="breed-list" value={r.breed} onChange={(e) => updateRow(r.key, 'breed', e.target.value)} placeholder="Breed / Plant" className="search-input text-xs w-full" style={{ padding: '7px 10px' }} />
                          </td>
                ...
                <div className="text-[10px] font-bold text-slate-400 mt-2 text-right">
                  ...
                  &nbsp;·&nbsp; Balance: <span className={`font-black ${remain < 0 ? 'text-red-600' : 'text-blue-700'}`}>{remain}</span>
                </div>
              </div>
              ...
              <div className="flex gap-3 justify-end pt-1 border-t border-slate-100">
                <button onClick={onClose} className="text-[10px] font-black text-slate-500 hover:text-slate-800 uppercase tracking-widest bg-slate-50 px-5 py-2.5 rounded-full border border-slate-200 cursor-pointer">Cancel</button>
